...
function Chat() {
    ...
    const handleChatStart = (user: { id: number; name: string; avatar?: string }) => {
        setCurrentChatUser(user);
        // Refresh conversations to ensure the new chat appears in sidebar
        fetchConversations();
    };
        ...
        <>
            <div className="drawer lg:drawer-open h-screen">
                ...
                <div className="drawer-content flex flex-col h-full">
                    {/* Navbar - Only show if NOT in chat window on mobile, or always on desktop if you want */}
                    {/* For simplicity, we hide the main navbar when in chat mode on mobile, but keep it structure */}
                    {!currentChatUser && (
                        <nav className="navbar w-full bg-base-300 flex-none">
                    ...
                    <div className="flex-1 overflow-hidden flex flex-col">
                        {currentChatUser ? (
                            <ChatWindow chatUser={currentChatUser} onBack={handleBackToFriends} />
                        ) : (
                            <div className="p-4 flex-1 overflow-y-auto">
                                ...
                                <div className="p-4">
                                    {isFriendsView ? (
                                        <FriendList onSwitch={toggleFriendsView} onChatStart={handleChatStart} />
                                    ) : (
                ...
                <div className="drawer-side is-drawer-close:overflow-visible z-20">
                    ...
                    <div className="flex min-h-full flex-col items-start bg-base-200 is-drawer-close:w-14 is-drawer-open:w-64 w-64 transition-all duration-300">
                        {/* User Profile Section */}
                        <div className="w-full flex flex-col items-center justify-center mt-6 mb-4 px-4">
                            <div className="avatar">
                                <div className="ring-primary ring-offset-base-100 w-16 rounded-full ring-2 ring-offset-2">
                                    <img src={currentUser?.avatar || "https://img.daisyui.com/images/stock/photo-1534528741775-53994a69daeb.webp"} alt={currentUser?.name || "User"} />
                                </div>
                            </div>
                            <p className="mt-2 font-semibold text-center is-drawer-close:hidden">{currentUser?.name || "Usuario"}</p>
                        </div>
                            ...
                            <ul className="menu w-full">
                                {conversations.map((conv) => (
                                    <ChatItem
                                        ...
                                        isActive={currentChatUser?.id === conv.user.id}
                                        onClick={() => handleChatStart(conv.user)}
                                    />
                                ))}
                            </ul>
                        </div>
